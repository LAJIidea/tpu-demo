...
int64_t top::ConvOp::getFLOPs() {
    ...
    return module::getNumElements(getOutput()) *
            (attr.kd * attr.kd * attr.kw * attr.ic / attr.groups * 2 + extra);
}

LogicalResult top::ConvOp::init(InferenceParameter &param) {
    auto conv = new Conv();
    auto attr = parseParam();
    conv->setup(param.inputs[0], param.inputs[1], param.inputs[2], param.inputs[0], attr);
    param.handle = (void *)conv;
    return success();
}

void top::ConvOp::deinit(InferenceParameter &param) {
    if (param.handle != nullptr) {
        auto conv = (Conv *)param.handle;
        delete conv;
        param.handle = nullptr;
    }
}

LogicalResult top::ConvOp::inference(InferenceParameter &param) {
    if (param.handle == nullptr) {
        return failure();
    }
    auto conv = (Conv *)param.handle;
...
LogicalResult top::ConvOp::backward_weight(InferenceParameter &param, InferenceParameter &b_param) {
    if (param.handle == nullptr) {
        return failure();
    }
    auto conv = (Conv *)param.handle;
    conv->run_backw(b_param.inputs[0], b_param.outputs[0]);
